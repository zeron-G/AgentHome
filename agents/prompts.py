...
- rest回复20体力·sleep回复50体力·eat消耗1食物回复{food_restore}体力
...
位置:({x},{y}) 体力:{energy}/100  装备:工具={tool_equipped} 绳子={rope_equipped}
...
def build_vision_grid(npc, world) -> str:
    ...
    legend = "图例: @=你 P=玩家 E=交易所 W=木 S=石 O=矿 F=食 H=草药 ♣=森 ▲=岩 ⌂=城 ·=草"
    return "\n".join(lines) + "\n" + legend
...
def build_npc_system_prompt(npc, world) -> str:
    ...
    if proposals:
        prop_lines = []
        for p in proposals:
            from_npc = world.get_npc(p.get("from_id", ""))
            from_name = from_npc.name if from_npc else p.get("from_id", "?")
            prop_lines.append(
                f"  - 来自{from_name}({p['from_id']}): 给出 {p['offer_qty']}{p['offer_item']} "
...
def build_npc_context(npc, world, rag_memories: str = "") -> tuple[str, bool]:
    ...
    status = _CTX_STATUS.format(
        ...
        tool_equipped="是" if npc.active_tool else "否",
        rope_equipped="是" if npc.active_rope else "否",
